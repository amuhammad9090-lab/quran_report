...
class ExportService {
  ...
  String _capaianLabel(SantriRecord r) {
    if (r.status == HafalanStatus.tahfizh) {
      return r.surahName != null ? '${r.status.label} - ${r.surahName}' : r.status.label;
    }
    return r.wafaLevel != null ? '${r.status.label} - ${r.wafaLevel!.label}' : r.status.label;
  }
  ...
  Future<File> exportExcel(
    ...
    void writeMerged(String text, xls.CellStyle style) {
      ...
      final end = xls.CellIndex.indexByColumnRow(columnIndex: _headers.length - 1, rowIndex: row);
